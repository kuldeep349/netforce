var items=[];
var main_site_url='http://globalsoftwebtechnologies.com/happy-days-global/';
(function($) {
...
function trigger_hover(obj){
        ...
        if(jQuery(obj).find('.wrap_content').eq(0).hasClass('no-bg')){
         ...
         if(jQuery(obj).find('.pop-up-content').eq(0).find('.loading-div').length){
             ...
             jQuery.ajax({
                url : Drupal.settings.basePath+'afl/user/get_meta_data/'+jQuery(obj).attr('id').replace('user_block_','')+'/'+jQuery(obj).attr('data-gid'),
                type : 'GET',
                async : false,
                success : function(response){
                    jQuery(obj).find('.pop-up-content').eq(0).find('.tooltip_profile_detaile').append(response);
...
function trigger_click(target,parent_id,obj,ajaxPath){
        ...
        if(parent_id.length && parent_id !='empty'){
          jQuery.ajax({
              //url : 'http://localhost/force_matrix/user/mygenealogy1/ajaxTree/'+ parent_id + '/1'+get_params,
              url : ajaxPath+'/'+ parent_id + '/1'+get_params,
              'type' : 'GET',
              'async' : false,
			  beforeSend: function () {
                    //$("#load").css("display", "block");
                    jQuery.loader("on", main_site_url+'admin_assets/images/default.svg');
                  },
              'success' : function(response){
			  ...
			  complete: function () {
                    ...
                    jQuery.loader("off", main_site_url+'admin_assets/images/default.svg');
                }
...
(function (jQuery) {
    jQuery.loadingBlockShow = function(opts) {
        var defaults = {
            imgPath: '/assets/images/icon.gif',
            imgStyle: {
                width: 'auto',
                textAlign: 'center',
                marginTop: '20%'
        ...
        jQuery.loadingBlockHide();

        var img = jQuery('<div><img src="' + defaults.imgPath + '"><div>' + defaults.text + '</div></div>');
        var block = jQuery('<div id="loading_block"></div>');

        block.css(defaults.style).appendTo('body');
        img.css(defaults.imgStyle).appendTo(block);
    };

    jQuery.loadingBlockHide = function() {
        jQuery('div#loading_block').remove();
    };
}(jQuery));
jQuery.loader = function (val, path) {
    if (val == "on") {
        jQuery(function () {
            jQuery.loadingBlockShow({
                imgPath: path,
                text: 'please wait...',
                style: {
                    position: 'fixed',
        ...
        setTimeout(jQuery.loadingBlockHide, 1000);
    }
};
