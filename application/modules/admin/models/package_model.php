...
class Package_Model extends Common_Model
{
  ...
  */
  public function getAllActivePackage($order_by_order=null)
  {
    if(!empty($order_by_order))
    {
      $packages=$this->db->select('*')->from('package')->where('status','1')->order_by('id',$order_by_order)->get()->result();
    }
    else 
    {
