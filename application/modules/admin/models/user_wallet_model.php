...
class User_Wallet_Model extends Common_Model
{
      ...
      public function getEwalletBalanceType($user_id,$type)
      {
           $res=$this->db->select('amount')->from('final_'.$type.'_wallet')->where('user_id', $user_id)->get()->row();
           $balance=(!empty($res->amount))?$res->amount:0;
           return $balance;   
      }//end method
      ...
      {
      	$result=$this->db->select(array('u.user_id', 'u.username', 'u.registration_date','u.auto_registration_date', 'u.rank_name', 'u.active_status', 'w.amount as balance'))
   ...
   /*end method*/ 
     ...
     */ 
     ...
     public function OtherWalletRequests()
     {
      $res=$this->db->select(array(
        'd.id', 
        'd.request_id', 
        ...
        // 'ww.amount as ewallet_amount'
         ...
         ->from('other_payment_requests as d')
         ->join('user_registration as u' ,'u.user_id=d.user_id')
        //  ->join('final_e_wallet as ww', 'ww.user_id=d.user_id')
         ->where(array('d.status'=>'0')) //statis 0 is used for pending request
         ->order_by('id', 'desc')
