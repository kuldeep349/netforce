...
class Setting extends Common_Controller
{
    ...
    public function manageStockist(){
        ...
        if ($this->input->server('REQUEST_METHOD') == 'POST') {
            ...
            $this->db->update('tbl_site_content', array(
                'content' => $content
            ), array(
                'id' => 1
            ));
            $this->session->set_flashdata("flash_msg", '<span class="text-semibold">Well done!</span> Manage Stockist Content is updated Successfully');

        }
        $data['title'] ="Manage Stockist";
        $data['header'] ="Manage Stockist";
        $data['content']=$this->db->select('*')->from('tbl_site_content')->where('id',1)->get()->row_array();
        _adminLayout("setting-mgmt/manageStockist", $data);
    }
    ...
    */
    ...
    public function marqueeManagement()
    {
        if(!empty($this->input->post('btn')))
        {
            $date_format='';  
            ...
            $id=$this->input->post('id');
            $this->db->update('tbl_marquee',array('text'=>$data),array('id'=> $id));
            $this->session->set_flashdata("flash_msg", '<span class="text-semibold">Well done!</span> Marquee is edited successfully');
            redirect(site_url()."admin/setting/marqueeManagement");    
            exit; 
        }//end empty if btn clicked here
        $data['text']=$this->setting_model->marqueeSetting();
        _adminLayout("setting-mgmt/marquee_settings",$data);
    }

    public function marqueeManagement2()
    {
        if(!empty($this->input->post('btn')))
        {
            $date_format='';  
            $data=$this->input->post('data');
            $this->db->update('tbl_marquee',array('text'=>$data),array('id'=>'2'));
            $this->session->set_flashdata("flash_msg", '<span class="text-semibold">Well done!</span> Marquee is edited successfully');
            redirect(site_url()."admin/setting/marqueeManagement2");    
            exit; 
        }//end empty if btn clicked here
        $data['text']=$this->setting_model->marqueeSetting2();
        _adminLayout("setting-mgmt/marquee_settings",$data);
    }
