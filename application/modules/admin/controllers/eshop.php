...
class Eshop extends Common_Controller 
{
	...
	public function allcollectionCategoryList()
	{
		 $result=$this->db->query("SELECT * FROM collection_center order by position")->result_array();	     	
		 $data['all_category']=$result;
		 $data['module_name']=$this->moduleName;
		 $data['controller_name']=$this->controllerName;
		_adminLayout("ecommerce/eshop-mgmt/all-collection-category",$data);
	}
	...
	public function addNewCategory()
	{
		if(!empty($this->input->post('btn')))
		{
			...
			echo "<script>window.location.href='".site_url().$this->moduleName."/eshop/allCategoryList"."'</script>";
			exit();
		}
	...
	public function addNewcollectionCategory()
	{
		if(!empty($this->input->post('btn')))
		{
			...
			$member_phone=$this->input->post('manager_phone');
			$member_email=$this->input->post('manager_email');
			//$active_status=$this->input->post('active_status');
			$date=date('d-M-Y');
			$role=$this->role;
			
			$position=$this->db->select_max('position')->from('collection_center')->get()->row();
			if(!empty($position->position))
			{
				$position=$position->position+1;
			}
            ...
            $this->db->insert('collection_center', $insert_data);
			$this->session->set_flashdata("flash_msg", '<span class="text-semibold">Category Added Successfully</span>');
			//redirect(site_url().$this->moduleName."/eshop/allCategoryList");
			echo "<script>window.location.href='".site_url().$this->moduleName."/eshop/allcollectionCategoryList"."'</script>";
			exit();
		}
		
		 $result=$this->db->query("SELECT * from collection_center")->result_array();
		 $data['module_name']=$this->moduleName;
		 $data['all_category']=$result;
		_adminLayout("ecommerce/eshop-mgmt/add-collection-category",$data);
	}
	public function deleteCollectionCategory($del_id)
	{
		    // redirect(site_url().'admin/eshop/allCategoryList');exit;
		    $del_id=ID_decode($del_id);
			$this->db->delete('collection_center', array('id' => $del_id));
			$this->session->set_flashdata("flash_msg", '<span class="text-semibold">Stockist Deleted Successfully</span>');
			//redirect(site_url().$this->moduleName."/eshop/allCategoryList");
			echo "<script>window.location.href='".site_url().$this->moduleName."/eshop/allcollectionCategoryList"."'</script>";
			exit();
	}
	...
	public function editCategory($fetch_id=null)
	{
		...
		public function editcollectionCategory($fetch_id=null)
	{
		if(!empty($this->input->post('btn')))
		{
           ...
           'manager_phone' => $member_phone,
			...
			$this->db->where('id', $hidid);
			$this->db->update('collection_center', $update_data);
			
			$this->session->set_flashdata("flash_msg", '<span class="text-semibold">Updated Successfully</span>');
		//	redirect(site_url().$this->moduleName."/eshop/allcollectionCategoryList");
			echo "<script>window.location.href='".site_url().$this->moduleName."/eshop/allcollectionCategoryList"."'</script>";
			exit();
		}
		
		 $fetch_id=ID_decode($fetch_id);
		 $result=$this->db->query("SELECT * from collection_center where id='".$fetch_id."'")->row_array();
		 $data['category_data']=$result;
		 
		 $result1=$this->db->query("SELECT * from collection_center where id!='".$fetch_id."'")->result_array();
		 $data['all_category']=$result1;
		 $data['module_name']=$this->moduleName;
		_adminLayout("ecommerce/eshop-mgmt/edit-collection-category",$data);    
	}
	...
	public function allcollectionSubCategoryList()
	{
		 $result=$this->db->select(array(
		 'sc.*',
		 'mc.category_name'
		 ))
		 ->from('collection_subcategory as sc')
		 ->join('collection_center as mc','mc.id=sc.parent_id')
		 ->order_by('sc.position','asc')
		 ->get()
		 ->result_array();
		 $data['all_subcategory']=$result;
		 $data['module_name']=$this->moduleName;
		 $data['controller_name']=$this->controllerName;
		 _adminLayout("ecommerce/eshop-mgmt/all-collection-subcategory",$data);
	}
	...
	public function addNewSubCategory()
	{
		...
		//pr($_POST); exit;	
			...
			echo "<script>window.location.href='".site_url().$this->moduleName."/eshop/allSubCategoryList"."'</script>";
			exit();
	...
	public function addNewcollectionSubCategory()
	{
		 if(!empty($this->input->post('btn')))
		{
		//pr($_POST); exit;	
			...
			$home_position_exist=$this->db->select('*')->from('collection_subcategory')->where(array('display_home_position'=>$display_home_position,'is_display_on_home'=>'1'))->get()->num_rows();
			//pr($home_position_exist);
			if($home_position_exist>0)
			{
				$this->session->set_flashdata("error_msg", '<span class="text-semibold">Sorry, this home page position is already exist</span>');
				redirect(site_url().$this->moduleName."/eshop/addNewcollectionSubCategory");
				exit();
			}
			...
			$position=$this->db->select_max('position')->from('collection_subcategory')->get()->row();
			if(!empty($position->position))
			{
				$position=$position->position+1;
			}
            ...
            $this->db->insert('collection_subcategory', $insert_data);
			
			$this->session->set_flashdata("flash_msg", '<span class="text-semibold">Sub Category Added Successfully</span>');
			//redirect(base_url()."admin/eshop/allSubCategoryList");
			echo "<script>window.location.href='".site_url().$this->moduleName."/eshop/allcollectionSubCategoryList"."'</script>";
			exit();
		 ...
		 $data['all_category']=$result;
		 $data['module_name']=$this->moduleName;
		_adminLayout("ecommerce/eshop-mgmt/add-collection-subcategory",$data);
	}
	...
	public function editcollectionSubCategory($fetch_id=null)
	{
		if(!empty($this->input->post('btn')))
		{
			$hidid=$this->input->post('hidid');
			...
			{
				$this->session->set_flashdata("error_msg", '<span class="text-semibold">Sorry, this home page position is already exist</span>');
				redirect(site_url().$this->moduleName."/eshop/editcollectionSubCategory/".ID_encode($hidid));
				exit();
			}
			...
			$this->db->where('id', $hidid);
			$this->db->update('collection_subcategory', $update_data);
		 ...
		 $fetch_id=ID_decode($fetch_id);
		 $result=$this->db->query("SELECT * from collection_subcategory where id='".$fetch_id."'")->row_array();
		 
		 $category=$this->db->query("SELECT * from collection_subcategory")->result_array();
		 
		 $data['subcategory_data']=$result;
		 $data['all_category']=$category;
		 $data['module_name']=$this->moduleName;
		 
		_adminLayout("ecommerce/eshop-mgmt/edit-collection-subcategory",$data);    
	}
	...
	public function allProductList()
	{
		 ...
		 foreach($data['all_products'] as $key => $product){
			$data['all_products'][$key]['userinfo'] = $this->db->query("SELECT username,first_name FROM user_registration WHERE user_id = '".$product['user_id']."'")->row_array();
		 }
		 $data['module_name']=$this->moduleName;
		_adminLayout("ecommerce/eshop-mgmt/all-products",$data);
	}
	public function allOrganicProductList()
	{
		 $result=$this->db->query("SELECT * from eshop_organic_products  order by id desc")->result_array();	  
		 //pr($result); exit;
		 $data['all_products']=$result;
		 $data['module_name']=$this->moduleName;
		_adminLayout("ecommerce/eshop-mgmt/all-organic",$data);
	}
	...
	public function addNewOrganicProduct()
	{
	    //pr($_POST); exit;
		if(!empty($this->input->post('btn')))
		{
            ...
            $this->db->insert('eshop_organic_products', $insert_data);
			$update_sub_img=serialize($update_sub_img);
			//////////////////////////////////
			$product_id=$this->db->insert_id();
			$this->db->update('eshop_products',array('sub_images'=>$update_sub_img),array('id'=>$product_id));
			...
			$this->session->set_flashdata("flash_msg", '<span class="text-semibold">Product Added Successfully</span>');
			//redirect(site_url().$this->moduleName."/eshop/allProductList");
			echo "<script>window.location.href='".site_url().$this->moduleName."/eshop/allOrganicProductList"."';</script>";
			exit();
		 ...
		 $data['all_category']=$result;
		 $data['module_name']=$this->moduleName;
		_adminLayout("ecommerce/eshop-mgmt/add-organic",$data);
	}
	
	public function editOrganicProduct($fetch_id=null)
	{
		if(!empty($this->input->post('btn')))
		{
			$hidid=$this->input->post('hidid');
            ...
            $this->db->where('id', $hidid);
			$this->db->update('eshop_organic_products', $update_data);
			...
			$this->db->update('eshop_organic_products',array('sub_images'=>$upload_sub_img),array('id'=>$hidid));
			
			$this->session->set_flashdata("flash_msg", '<span class="text-semibold">Product Updated Successfully</span>');
			redirect(site_url().$this->moduleName."/eshop/allOrganicProductList");
			exit();
			
		}
		
		 $fetch_id=ID_decode($fetch_id);
		 $result=$this->db->query("SELECT * from eshop_organic_products where id='".$fetch_id."'")->row_array();
		 $data['product_data']=$result;
		 $data['sub_images']=unserialize($result['sub_images']);
		 ...
		 $data['sub_category']=$sub_category;
		 $data['module_name']=$this->moduleName;
		_adminLayout("ecommerce/eshop-mgmt/edit-organic",$data);
	}
