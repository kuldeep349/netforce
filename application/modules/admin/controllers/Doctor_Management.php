...
*/
class Doctor_Management extends Common_Controller 
{
	private $moduleName;
	...
	public function allDocCategoryList()
	{
		 $result=$this->db->query("SELECT * FROM doctor_category order by position")->result_array();	     	
		 $data['all_category']=$result;
		 $data['module_name']=$this->moduleName;
		 $data['controller_name']=$this->controllerName;
		_adminLayout("doctor-management/doctor-mgmt/all-category",$data);
	}
	public function addNewDocCategory()
	{
		if(!empty($this->input->post('btn')))
		{
		 ...
		 $data['module_name']=$this->moduleName;
		 $data['all_category']=$result;
		_adminLayout("doctor-management/doctor-mgmt/add-category",$data);
	}
	public function allSubDocCategoryList()
	{
		 $result=$this->db->select(array(
		 'sc.*',
		 'mc.category_name'
		 ))
		 ->from('doctor_subcategory as sc')
		 ->join('doctor_category as mc','mc.cat_id=sc.parent_id')
		 ...
		 $data['module_name']=$this->moduleName;
		 $data['controller_name']=$this->controllerName;
		 _adminLayout("doctor-management/doctor-mgmt/all-subcategory",$data);
	}
	public function addNewDocSubCategory()
	{
		if(!empty($this->input->post('btn')))
		{
		//pr($_POST); exit;	
			...
			$home_position_exist=$this->db->select('*')->from('doctor_subcategory')->where(array('display_home_position'=>$display_home_position,'is_display_on_home'=>'1'))->get()->num_rows();
            ...
            $this->db->insert('doctor_subcategory', $insert_data);
		 ...
		 $data['all_category']=$result;
		 $data['module_name']=$this->moduleName;
		_adminLayout("doctor-management/doctor-mgmt/add-subcategory",$data);
	}
    ...
    public function deleteCategory($del_id)
	{
		   // redirect(site_url().'admin/eshop/allCategoryList');exit;
			...
			$this->db->delete('austin_doctor', array('category_id' => $del_id));
			$this->db->delete('doctor_subcategory', array('parent_id' => $del_id));
			$this->db->delete('doctor_category', array('cat_id' => $del_id));
	...
	{
		    $del_id=ID_decode($del_id);
             $this->db->delete('austin_doctor', array('parent_category_id' => $del_id));	
			
			$this->db->delete('doctor_subcategory', array('sub_cat_id' => $del_id));	
	...
	{
		 $result=$this->db->query("SELECT * from austin_doctor order by doctor_id desc")->result_array();	     	
	...
	function getAjaxSubCategory()
	{
		...
		$result=$this->db->select('*')->from('doctor_subcategory')->where(array('parent_id'=>$parent_category_id,'active_status'=>'1'))->get()->result();
	...
	public function addNewDoctor()
	{
		if(!empty($this->input->post('btn')))
		{
            ...
            $this->db->insert('austin_doctor', $insert_data);
            $doctot_id=$this->db->insert_id();
            $this->db->delete('doctor_time', array('doctor_id' => $doctot_id));
            foreach ($_POST['DoctorTime'] as $key => $value) {
            	$time_data=array(
            		'doctor_id'=>$doctot_id,
            		'available_time'=>$value
            	);
            	$this->db->insert('doctor_time', $time_data);
            }
		...
		$result=$this->db->query("SELECT * from doctor_category")->result_array();
		 $data['all_category']=$result;
		$data['module_name']=$this->moduleName;
		_adminLayout("doctor-management/doctor-mgmt/add-doctor",$data);
	}
	function editDoctor($fetch_id=null)
	{
		$fetch_id=ID_decode($fetch_id);
		if(!empty($this->input->post('btn')))
		{
			...
			$this->db->delete('doctor_time', array('doctor_id' => $hidid));
            foreach ($_POST['DoctorTime'] as $key => $value) {
            	$time_data=array(
            		'doctor_id'=>$hidid,
            		'available_time'=>$value
		...
		$result=$this->db->query("SELECT * from doctor_category")->result_array();
		 $data['all_category']=$result;
		 $data['doctor_time']=$this->db->query("SELECT * from doctor_time where doctor_id='".$fetch_id."'")->result_array();
		$data['module_name']=$this->moduleName;
		_adminLayout("doctor-management/doctor-mgmt/edit-doctor",$data);
	}
	function deleteDoctor($fetch_id=null)
	{
		$del_id=ID_decode($fetch_id);
		$this->db->delete('austin_doctor', array('doctor_id' => $del_id));	
		$this->session->set_flashdata("flash_msg", '<span class="text-semibold">Sub Category Deleted Successfully</span>');
			echo "<script>window.location.href='".site_url().$this->moduleName."/Doctor_Management/allDoctorList"."'</script>";
			exit();
	}
