...
class Eshop_Orders extends Common_Controller 
{
	...
	{	  	 
		...
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('user_id'=>$this->user_id,'order_type' => '1'))->order_by('id','desc')->get()->result();
		//pr($all_order);
	...
	{	  	 
		$data['title']='All Orders';
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('user_id'=>$this->user_id,'order_type' => '2'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['order_type']='all';
		$data['module_name']=$this->moduleName;
	...
	public function allPendingOrders()
	{
		...
		$data['module_name']=$this->moduleName;
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('order_status'=>'0','user_id'=>$this->user_id,'order_type' => '1'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['module_name']=$this->moduleName;
	...
	public function allConfirmedOrder()
	{
		...
		$data['module_name']=$this->moduleName;
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('order_status'=>'1','user_id'=>$this->user_id,'order_type' => '1'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['module_name']=$this->moduleName;
	...
	public function allRejectedOrders()
	{
		...
		$data['module_name']=$this->moduleName;
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('order_status'=>'2','user_id'=>$this->user_id,'order_type' => '1'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['module_name']=$this->moduleName;
	...
	public function allPendingeOrders()
	{
		$data['title']='Pending Orders';	
		$data['module_name']=$this->moduleName;
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('order_status'=>'0','user_id'=>$this->user_id,'order_type' => '2'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['module_name']=$this->moduleName;
		$data['order_type']='pending';
	...
	public function allConfirmedeOrder()
	{
		$data['title']='Confirmed Orders';		
		$data['module_name']=$this->moduleName;
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('order_status'=>'1','user_id'=>$this->user_id,'order_type' => '2'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['module_name']=$this->moduleName;
		$data['order_type']='confirmed';
	...
	public function allRejectedeOrders()
	{
		$data['title']='Rejected Orders';		
		$data['module_name']=$this->moduleName;
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('order_status'=>'2','user_id'=>$this->user_id,'order_type' => '2'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['module_name']=$this->moduleName;
		$data['order_type']='rejected';
	...
	public function allDeliveredeOrders()
	{
		$data['title']='Delivered Orders';		
		$data['module_name']=$this->moduleName;
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('order_status'=>'3','user_id'=>$this->user_id,'order_type' => '2'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['module_name']=$this->moduleName;
	...
	public function allDeliveredOrders()
	{
		$data['title']='Delivered Orders';		
		$data['module_name']=$this->moduleName;
		$all_order=$this->db->select('*')->from('eshop_organic_orders')->where(array('order_status'=>'3','user_id'=>$this->user_id,'order_type' => '1'))->order_by('id','desc')->get()->result();
		//pr($all_order);
		$data['all_orders']=$all_order;
		$data['module_name']=$this->moduleName;
		$data['order_type']='delivered';
	...
	?>
		<div class="panel panel-white" >
               ...
               <div class="panel-body">
                  ...
                  <p class="text-muted">Thank you from Xbulon</p>
                  <a href="javascript:print_invoice();" class="btn btn-success">Print</a> <button onclick="print_invoice()" class="btn btn-success">Download</button>
               </div>
        </div>	
	<?php 
	}//end function
