	...
	public function purchasePin()
	{
		if(!empty($this->input->post('btn')))
		{
			...
			if(!empty($pkg_id_array) && count($pkg_id_array)>0)
			{
				...
				if($payment_method=='ewallet')
				{
				    $this->db->update('epin',array('request_status'=>'1','response_date'=>date('Y-m-d H:i:s')),array('request_id'=>$request_id,'user_id'=>$this->userId));
                	$epin_info=$this->db->select('*')->from('epin')->where(array('request_id'=>$request_id,'user_id'=>$this->userId))->get()->row();
		    ...
		    if($payment_method=='ewallet')
			{
                $this->session->set_flashdata("flash_msg",'<h5 class="panel-title" style="color:green">Your Epin generated successfully!</h5>');
		        redirect(site_url().'user/epin/approvedEpinRequest');
			}
			else
			{
