   ...
   <div class="page-header page-header-default">
      <div class="page-header-content">
         <div class="page-title">
            <h4><i class="icon-arrow-left52 position-left"></i> <span class="text-semibold">Eshopping</span> - Product</h4>
         </div>
...
{
    var payment_mode= $("#stockist").val();
    //alert(payment_mode);
    window.location.href='<?php echo site_url().$module_name."/eshop/generateInvoice2/ewallet/";?>'+payment_mode + '/shop';
}
function proceedtopay1()
{
    ...
    {
    window.location.href='<?php echo site_url().$module_name."/eshop/paystack/";?>';
    }
   if(payment_mode=='bank'){
      alert('Not Available');
      return;
   }
   if(payment_mode=='perfect_money'){
         alert('Not Available');
         return;
   }
...
function removefromcart(query)
{
    //alert(query);
    $.ajax({
   url:"<?php echo base_url().$module_name; ?>/cart/removeItemFromCart2",
   method:"POST",
   data:{query:query},
   success:function(response){
       //$('#result').html(response);
...
function updatecart(query,qty)
{
   //alert(query+'--'+qty);
   $.ajax({
   url:"<?php echo base_url().$module_name; ?>/cart/updateItemInCart2",
   method:"POST",
