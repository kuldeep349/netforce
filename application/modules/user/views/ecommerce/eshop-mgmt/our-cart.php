...
function proceedtopay()
{
    ...
    window.location.href='<?php echo site_url().$module_name."/eshop/generateInvoice/ewallet/";?>'+payment_mode + '/organic';
}
function proceedtopay1()
{
    ...
    if(payment_mode=='product_ewallet')
    {
        //check if product come under category capsule then does not allow to product wallet
        $class = $("input[name='payment_mode']:checked").attr('class');
        if($class=='1')
        {
            alert("Not Allow");
            return;
        }
    window.location.href='<?php echo site_url().$module_name."/eshop/ProductewalletPayment/";?>';
    }
