...
*/
function stageWiseMatrixUnilevelLevelCommision()
	{
		$obj=& get_instance();
        //////
		...
		$matrix_stage_level_commission=$obj->db->select('id')->from(' unilevel_stage_level_commission')->where('pkg_id',$pkg_id)->get();
		...
		if($matrix_stage_level_commission->num_rows()>0)
        {
			...
			$obj->db->update("unilevel_stage_level_commission",$data,array('pkg_id'=>$pkg_id));
			
				
			$obj->db->delete('unilevel_stage_level_commission_meta', array('pkg_id' =>$pkg_id));
			
			insertIntoStageWiseUnilevelCommissionMeta($pkg_id);   
        }
        else 
        {
			$data=array('pkg_id'=>$pkg_id,'commission_type'=>$commission_type);
			
			$obj->db->insert("unilevel_stage_level_commission",$data);
			insertIntoStageWiseUnilevelCommissionMeta($pkg_id);
        }
	}//end function here
function insertIntoStageWiseUnilevelCommissionMeta($pkg_id)
{
		$obj=& get_instance();
		...
		$stage_7_level_commission_array=$obj->input->post('stage_7_level_commission');
		
		for($i=0;$i<count($stage_1_level_commission_array);$i++)
		{
			if(!empty($stage_1_level_commission_array[$i]))
			{
				$obj->db->insert("unilevel_stage_level_commission_meta",array(
				'pkg_id'=>$pkg_id,
				'stage_no'=>'1',
		...
		{
			if(!empty($stage_2_level_commission_array[$i]))
			{
				$obj->db->insert("unilevel_stage_level_commission_meta",array(
				'pkg_id'=>$pkg_id,
				'stage_no'=>'2',
		...
		{
			if($stage_3_level_commission_array[$i])
			{
				$obj->db->insert("unilevel_stage_level_commission_meta",array(
				'pkg_id'=>$pkg_id,
				'stage_no'=>'3',
		...
		{
			if($stage_4_level_commission_array[$i])
			{
				$obj->db->insert("unilevel_stage_level_commission_meta",array(
				'pkg_id'=>$pkg_id,
				'stage_no'=>'4',
		...
		{
			if($stage_5_level_commission_array[$i])
			{
				$obj->db->insert("unilevel_stage_level_commission_meta",array(
				'pkg_id'=>$pkg_id,
				'stage_no'=>'5',
		...
		{
			if($stage_6_level_commission_array[$i])
			{
				$obj->db->insert("unilevel_stage_level_commission_meta",array(
				'pkg_id'=>$pkg_id,
				'stage_no'=>'6',
		...
		for($i=0;$i<count($stage_7_level_commission_array);$i++)
		{
			if(!empty($stage_7_level_commission_array[$i]))
			{
				$obj->db->insert("unilevel_stage_level_commission_meta",array(
				'pkg_id'=>$pkg_id,
				'stage_no'=>'7',
				'stage_key'=>'stage_6',
				'level'=>$i+1,
				'commission_amount'=>$stage_7_level_commission_array[$i]
				));				
			}
		}
}//end function	
